...
{return(i.arrival<j.arrival||(i.arrival==j.arrival&&i.burst<j.burst));
}
int main()
{
int n;
...
cout<<"------------------------------------------"<<endl;
for(int i=0;i<n;i++)
cout<<i+1<<"\t |"<<jobs[i].arrival<<"\t |"<<jobs[i].burst<<"\t |"<<jobs[i].end<<"\t
|"<<jobs[i].waiting<<endl;
cout<<"Turn arround time : "<<(double)tat/n<<" Average waiting time :
"<<(double)awt/n<<endl;
